...
class ReorderController extends ControllerBehavior
{
    ...
    public function reorderGetRecordName($record)
    {
        ...
        $result = $record->{$this->nameFrom};         

        return $result; 
    }

    public function reorderGetRecordID($record)
    {
        return $record->id;
    }

    public function reorderGetRecordColor($record)
    {
        $color = null;
        if($record->main_color){
            $color = $record->main_color;
        }
        return $record->main_color;
    }    

    public function reorderGetRecordUpdateURL($record)
    {
        $actionurl = $this->controller->actionUrl('update');
        $actionurl = $actionurl.'/'.$record->id;
        if($this->controller->prefilter){
            $actionurl = $actionurl.'/'.$this->controller->prefilter;          
        }
        return $actionurl;
    }

    public function onDeleteReorder(){
