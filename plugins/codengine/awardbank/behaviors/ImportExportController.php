<?php namespace Codengine\Awardbank\Behaviors;
...
use Codengine\Awardbank\Behaviors\ImportExportController\TranscodeFilter;
...
class ImportExportController extends ControllerBehavior
{
    ...
    public function onImport() {
        try {
            ...
            /**
             * Pass offset and chunk size
             */
            $importOptions['offset'] = post('offset', 0);
            $importOptions['chunkSize'] = $this->chunkSize;
            $importOptions['totalRows'] = Session::get('importResults.totalRows');
        ...
        return [
            'result' => $this->makePartial('import_result_form'),
            'data'   => $this->vars
        ];
    }
    ...
    public function prepareImportVars()
    {
        ...
        // Remove previous importResults and data
        Session::forget('importResults');

        // Get total rows count and put it to session
        Session::put('importResults.totalRows', $this->getImportTotalRows());
    ...
    /**
     * Needed for import progress percentage calculation
     *
     * @return int Total rows count in CSV file
     */
    protected function getImportTotalRows() {
        if (!$path = $this->getImportFilePath()) {
            return null;
        }
    ...
    protected function makeImportUploadFormWidget()
    {
        ...
        $widgetConfig = $this->makeConfig('~/plugins/codengine/awardbank//behaviors/importexportcontroller/partials/fields_import.yaml');
    ...
    protected function makeExportFormatFormWidget()
    {
        ...
        $widgetConfig = $this->makeConfig('~/plugins/codengine/awardbank//behaviors/importexportcontroller/partials/fields_export.yaml');
    ...
    public function exportFromList($definition = null, $options = [])
    {
        ...
        foreach ($results->toArray() as $key1 => $result) {
            $record = [];
            $columnToCheck = $result1->get($key1);
            foreach ($columns as $key=> $column) {
                if($key == 'feature_image'){
                    if(!empty($result['feature_image'])){
                        $image = $result['feature_image']->file_name;
                        $record[] = $image;
                    } else if (!empty($result['external_image_url'])){
                        $record[] = $result['external_image_url'];
                    } else {
                        $record[] = 'No Image';
                    }
                } else if($key == 'primary_color'){
                    $record[] = $result['primary_color'];
                } else if($key == 'secondary_color'){
                    $record[] = $result['secondary_color'];
            ...
            $resultsFinal[$key1] = $record;
        }
        $csv->insertAll($resultsFinal);
        $time = microtime(true) - $start;
        \Log::info('loop 1 time : '.$time);
        /*
         * Output
         */
